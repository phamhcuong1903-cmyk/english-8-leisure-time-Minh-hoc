...
interface VerbFormQuestionProps {
...
export const VerbFormQuestion: React.FC<VerbFormQuestionProps> = ({
  question,
  index,
  value,
  onChange,
  ...
  const verbHint = question.verbHint || '';
    ...
    >
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <span className="text-xs font-bold px-2.5 py-1 rounded-md bg-[#E0E7FF] text-[#4F46E5]">
            Câu {index + 1}
          </span>
          {verbHint && (
            <span className="inline-block text-xs font-bold text-[#4F46E5] bg-indigo-50 border border-indigo-100 px-2 py-0.5 rounded">
              Động từ gợi ý: {verbHint}
            </span>
          )}
        </div>
        {isGraded && (
        ...
        {parts.length >= 2 ? (
            ...
            <input
              ...
              className={`inline-block mx-1.5 px-3 py-1 border-b-2 font-semibold text-center transition-all outline-none rounded-md ${
                isGraded
                  ...
                  : 'border-[#4F46E5] bg-[#FDFBF7] text-[#334155] focus:bg-white focus:border-[#F59E42] focus:ring-2 focus:ring-[#F59E42]/20'
              }`}
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => onChange(e.target.value)}
              disabled={isGraded}
              placeholder="Chia động từ tại đây..."
